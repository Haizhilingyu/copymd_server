...
router.post('/url', async (ctx, next) => {
    ...
    if (isStringNotEmpty(markdown) && isStringNotEmpty(url) && isStringNotEmpty(password)) {
        const cacheData = await getData(url)
        if (cacheData && cacheData.expiration && cacheData.expiration > Date.now() && cacheData.markdown) {
...
async function saveData(markdown, url, password, isPrivate) {
    try {
        ...
        await kv.set(url, { markdown, password, expiration, isPrivate }, { ex: expirationTime });
    } catch (error) {
        // Handle errors
    }
}
